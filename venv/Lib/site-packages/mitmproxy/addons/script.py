...
class Script:
    ...
    def loadscript(self):
        ...
        if self.ns:
            ctx.main.addons.remove(self.ns)
        self.ns = None
        with addonmanager.safecall():
            ns = load_script(self.fullpath)
            ctx.main.addons.register(ns)
            self.ns = ns
        if self.ns:
            # We're already running, so we have to explicitly register and
            # configure the addon
            ctx.main.addons.invoke_addon(self.ns, "running")
            ctx.main.addons.invoke_addon(
                self.ns,
                "configure",
                ctx.options.keys()
            )
...
class ScriptLoader:
    ...
    def script_run(self, flows: typing.Sequence[flow.Flow], path: mtypes.Path) -> None:
        ...
        if mod:
            with addonmanager.safecall():
                ctx.main.addons.invoke_addon(mod, "running")
                ctx.main.addons.invoke_addon(
                    mod,
                    "configure",
                    ctx.options.keys()
                ...
                for f in flows:
                    for evt, arg in eventsequence.iterate(f):
                        ctx.main.addons.invoke_addon(mod, evt, arg)
    ...
    def configure(self, updated):
        if "scripts" in updated:
            ...
            for s in newscripts:
                ctx.main.addons.register(s)
                if self.is_running:
                    # If we're already running, we configure and tell the addon
                    # we're up and running.
                    ctx.main.addons.invoke_addon(s, "running")
