...
class Channel:
    """
        The only way for the proxy server to communicate with the main
        is to use the channel it has been given.
    """
    def __init__(self, main, loop, should_exit):
        self.main = main
        self.loop = loop
    ...
    def ask(self, mtype, m):
        """
        Decorate a message with a reply attribute, and send it to the main.
        ...
        if not self.should_exit.is_set():
            ...
            asyncio.run_coroutine_threadsafe(
                self.main.addons.handle_lifecycle(mtype, m),
                self.loop,
            )
            g = m.reply.q.get()
    ...
    def tell(self, mtype, m):
        """
        Decorate a message with a dummy reply attribute, send it to the main,
