...
class CookieAttributeEditor(base.FocusEditor):
    ...
    def layout_pushed(self, prev):
        if self.grideditor.main.view.focus.flow:
            self._w = base.BaseGridEditor(
                self.grideditor.main,
                self.title,
                self.columns,
...
class OptionsEditor(base.GridEditor, layoutwidget.LayoutWidget):
    ...
    def __init__(self, main, name, vals):
        self.name = name
        super().__init__(main, [[i] for i in vals], self.callback)

    def callback(self, vals):
        try:
            setattr(self.main.options, self.name, [i[0] for i in vals])
...
class DataViewer(base.GridEditor, layoutwidget.LayoutWidget):
    ...
    def __init__(
        ...
        super().__init__(main, vals, self.callback)
