...
class SimpleOverlay(urwid.Overlay, layoutwidget.LayoutWidget):
    ...
    def __init__(self, main, widget, parent, width, valign="middle"):
        self.widget = widget
        self.main = main
        super().__init__(
            widget,
...
class Chooser(urwid.WidgetWrap, layoutwidget.LayoutWidget):
    ...
    def __init__(self, main, title, choices, current, callback):
        self.main = main
        self.choices = choices
    ...
    def keypress(self, size, key):
        key = self.main.keymap.handle_only("chooser", key)
        ...
        binding = self.main.keymap.get("global", key)
        ...
        if binding and binding.command.startswith("console.nav"):
            self.main.keymap.handle("global", key)
...
class OptionsOverlay(urwid.WidgetWrap, layoutwidget.LayoutWidget):
    ...
    def __init__(self, main, name, vals, vspace):
        """
            vspace: how much vertical space to keep clear
        """
        cols, rows = main.ui.get_cols_rows()
        self.ge = grideditor.OptionsEditor(main, name, vals)
        super().__init__(
            urwid.AttrWrap(
                urwid.LineBox(
...
class DataViewerOverlay(urwid.WidgetWrap, layoutwidget.LayoutWidget):
    ...
    def __init__(self, main, vals):
        """
            vspace: how much vertical space to keep clear
        """
        cols, rows = main.ui.get_cols_rows()
        self.ge = grideditor.DataViewer(main, vals)
        super().__init__(
            urwid.AttrWrap(
                urwid.LineBox(
