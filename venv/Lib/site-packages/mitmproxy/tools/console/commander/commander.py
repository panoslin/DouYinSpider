...
class CommandBuffer:
    def __init__(self, main: mitmproxy.main.Main, start: str = "") -> None:
        self.main = main
    ...
    def parse_quoted(self, txt):
        parts, remhelp = self.main.commands.parse_partial(txt)
    ...
    def cycle_completion(self) -> None:
        if not self.completion:
            parts, remainhelp = self.main.commands.parse_partial(self.text[:self.cursor])
            last = parts[-1]
            ...
            if ct:
                self.completion = CompletionState(
                    completer = ListCompleter(
                        parts[-1].value,
                        ct.completion(self.main.commands, last.type, parts[-1].value)
                    ),
                    parse = parts,
                )
        if self.completion:
...
class CommandEdit(urwid.WidgetWrap):
    ...
    def __init__(self, main: mitmproxy.main.Main, text: str) -> None:
        super().__init__(urwid.Text(self.leader))
        self.main = main
        self.cbuf = CommandBuffer(main, text)
        self.update()
