...
class ConsoleAddon:
    ...
    def options_reset_current(self) -> None:
        ...
        """
        fv = self.main.window.current("options")
        if not fv:
            raise exceptions.CommandError("Not viewing options.")
        self.main.commands.execute("options.reset.one %s" % fv.current_name())
    ...
    def nav_pagedown(self) -> None:
        """
            Go down.
        """
        self.main.inject_key("page down")
    ...
    def console_choose(
        ...
        *args: mitmproxy.types.Arg
        ...
        def callback(opt):
            ...
            try:
                self.main.commands.execute(repl)
            except exceptions.CommandError as e:
                signals.status_message.send(message=str(e))

        self.main.overlay(
            overlay.Chooser(self.main, prompt, choices, "", callback)
        )
    ...
    def console_choose_cmd(
        ...
        *args: mitmproxy.types.Arg
        ...
        """
        choices = ctx.main.commands.call_strings(choicecmd, [])
        ...
        def callback(opt):
            ...
            repl = repl.replace("{choice}", opt)
            try:
                self.main.commands.execute(subcmd + " " + repl)
            except exceptions.CommandError as e:
                signals.status_message.send(message=str(e))
    ...
    def console_command_set(self, option: str) -> None:
        ...
        """
        option_value = getattr(self.main.options, option, None)
        current_value = option_value if option_value else ""
        self.main.commands.execute(
    ...
    def view_flow(self, flow: flow.Flow) -> None:
        ...
        if hasattr(flow, "request"):
            # FIME: Also set focus?
            self.main.switch_view("flowview")
    ...
    def bodyview(self, f: flow.Flow, part: str) -> None:
        ...
        self.main.spawn_external_viewer(content, t)
    ...
    def edit_focus(self, part: str) -> None:
        ...
        """
        flow = self.main.view.focus.flow
        ...
        if part == "cookies":
            self.main.switch_view("edit_focus_cookies")
        elif part == "form":
            self.main.switch_view("edit_focus_form")
        elif part == "path":
            self.main.switch_view("edit_focus_path")
        elif part == "query":
            self.main.switch_view("edit_focus_query")
        elif part == "request-headers":
            self.main.switch_view("edit_focus_request_headers")
        elif part == "response-headers":
            self.main.switch_view("edit_focus_response_headers")
        elif part in ("request-body", "response-body"):
            if part == "request-body":
                message = flow.request
            else:
                message = flow.response
            c = self.main.spawn_editor(message.get_content(strict=False) or b"")
            ...
            self.main.switch_view("edit_focus_setcookies")
    ...
    def _grideditor(self):
        gewidget = self.main.window.current("grideditor")
        if not gewidget:
    ...
    def flowview_mode_set(self, mode: str) -> None:
        ...
        """
        fv = self.main.window.current_window("flowview")
        if not fv:
        ...
        try:
            self.main.commands.call_strings(
                "view.setval",
    ...
    def flowview_mode(self) -> str:
        ...
        if not fv:
            raise exceptions.CommandError("Not viewing a flow.")
        idx = fv.body.tab_offset
        return self.main.commands.call_strings(
            "view.getval",
            [
                "@focus",
                "flowview_mode_%s" % idx,
                self.main.options.console_default_contentview,
            ]
        )
    ...
    def key_unbind(self, contexts: typing.Sequence[str], key: str) -> None:
        ...
        """
        try:
            self.main.keymap.remove(key, contexts)
        except ValueError as v:
            raise exceptions.CommandError(v)

    def _keyfocus(self):
        kwidget = self.main.window.current("keybindings")
        if not kwidget:
    ...
    def key_unbind_focus(self) -> None:
        ...
        b = self._keyfocus()
        try:
            self.main.keymap.remove(b.key, b.contexts)
        except ValueError as v:
            raise exceptions.CommandError(v)
