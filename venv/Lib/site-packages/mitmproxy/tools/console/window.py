...
class WindowStack:
    def __init__(self, main, base):
        self.main = main
        self.windows = dict(
            flowlist = flowlist.FlowListBox(main),
            flowview = flowview.FlowView(main),
            commands = commands.Commands(main),
            keybindings = keybindings.KeyBindings(main),
            options = options.Options(main),
            help = help.HelpView(main),
            eventlog = eventlog.EventLog(main),

            edit_focus_query = grideditor.QueryEditor(main),
            edit_focus_cookies = grideditor.CookieEditor(main),
            edit_focus_setcookies = grideditor.SetCookieEditor(main),
            edit_focus_setcookie_attrs = grideditor.CookieAttributeEditor(main),
            edit_focus_form = grideditor.RequestFormEditor(main),
            edit_focus_path = grideditor.PathEditor(main),
            edit_focus_request_headers = grideditor.RequestHeaderEditor(main),
            edit_focus_response_headers = grideditor.ResponseHeaderEditor(main),
        )
        self.stack = [base]
        self.overlay = None
...
class Window(urwid.Frame):
    def __init__(self, main):
        self.statusbar = statusbar.StatusBar(main)
        super().__init__(
            None,
            header = None,
        ...
        self.main = main
        self.main.view.sig_view_refresh.connect(self.view_changed)
        self.main.view.sig_view_add.connect(self.view_changed)
        self.main.view.sig_view_remove.connect(self.view_changed)
        self.main.view.sig_view_update.connect(self.view_changed)
        self.main.view.focus.sig_change.connect(self.view_changed)
        self.main.view.focus.sig_change.connect(self.focus_changed)
        ...
        self.main.options.subscribe(self.configure, ["console_layout"])
        self.main.options.subscribe(self.configure, ["console_layout_headers"])
        self.pane = 0
        self.stacks = [
            WindowStack(main, "flowlist"),
            WindowStack(main, "eventlog")
        ]
    ...
    def refresh(self):
        """
            Redraw the layout.
        """
        c = self.main.options.console_layout
        if c == "single":
            self.pane = 0

        def wrapped(idx):
            widget = self.stacks[idx].top_widget()
            if self.main.options.console_layout_headers:
    ...
    def flow_changed(self, sender, flow):
        if self.main.view.focus.flow:
            if flow.id == self.main.view.focus.flow.id:
                self.focus_changed()
    ...
    def switch(self):
        ...
        """
        if self.main.options.console_layout == "single":
            self.pane = 0
        else:
    ...
    def keypress(self, size, k):
        ...
        if k:
            return self.main.keymap.handle(
