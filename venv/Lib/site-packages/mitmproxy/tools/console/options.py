...
class OptionItem(urwid.WidgetWrap):
    ...
    def get_widget(self):
        ...
        changed = self.walker.main.options.has_changed(self.opt.name)
        if self.focused:
...
class OptionListWalker(urwid.ListWalker):
    def __init__(self, main):
        ...
        self.opts = sorted(main.options.keys())
        self.maxlen = max(len(i) for i in self.opts)
        self.editing = False
        self.set_focus(0)
        self.main.options.changed.connect(self.sig_mod)

    def sig_mod(self, *args, **kwargs):
        self.opts = sorted(self.main.options.keys())
        self.maxlen = max(len(i) for i in self.opts)
        self._modified()
    ...
    def _get(self, pos, editing):
        name = self.opts[pos]
        opt = self.main.options._options[name]
        return OptionItem(
...
class OptionsList(urwid.ListBox):
    def __init__(self, main):
        self.main = main
        self.walker = OptionListWalker(main)
        super().__init__(self.walker)

    def save_config(self, path):
        try:
            optmanager.save(self.main.options, path)
    ...
    def keypress(self, size, key):
        if self.walker.editing:
            if key == "enter":
                ...
                try:
                    d = self.main.options.parse_setval(foc.opt.name, v)
                    self.main.options.update(**{foc.opt.name: d})
            ...
            if key == "m_start":
                ...
                if foc.opt.typespec == bool:
                    self.main.options.toggler(foc.opt.name)()
                    ...
                    self.main.overlay(
                        overlay.Chooser(
                            ...
                            foc.opt.choices,
                            foc.opt.current(),
                            self.main.options.setter(foc.opt.name)
                        )
                    )
...
class OptionHelp(urwid.Frame):
    ...
    def widget(self, txt):
        cols, _ = self.main.ui.get_cols_rows()
...
class Options(urwid.Pile, layoutwidget.LayoutWidget):
    ...
    def __init__(self, main):
        oh = OptionHelp(main)
        self.optionslist = OptionsList(main)
        super().__init__(
            [
                self.optionslist,
                (HELP_HEIGHT, oh),
