...
class EventLog(urwid.ListBox, layoutwidget.LayoutWidget):
    ...
    def __init__(self, main):
        ...
        self.walker = LogBufferWalker(
            collections.deque(maxlen=self.main.events.size)
        )

        main.events.sig_add.connect(self.add_event)
        main.events.sig_refresh.connect(self.refresh_events)
        self.main.options.subscribe(self.refresh_events, ["console_eventlog_verbosity"])
        self.refresh_events()
    ...
    def add_event(self, event_store, entry: log.LogEntry):
        if log.log_tier(self.main.options.console_eventlog_verbosity) < log.log_tier(entry.level):
            return
        ...
        if self.main.options.console_focus_follow:
            self.walker.set_focus(len(self.walker) - 1)

    def refresh_events(self, *_):
        self.walker.clear()
        for event in self.main.events.data:
