...
import mitmproxy.tools.web.main # noqa
...
class RequestHandler(tornado.web.RequestHandler):
    ...
    def view(self) -> "mitmproxy.addons.view.View":
        return self.application.main.view

    @property
    def main(self) -> "mitmproxy.tools.web.main.WebMain":
        return self.application.main
...
class DumpFlows(RequestHandler):
    ...
    def post(self):
        ...
        for i in io.FlowReader(bio).stream():
            asyncio.ensure_future(self.main.load_flow(i))
        bio.close()
...
class ReplayFlow(RequestHandler):
    def post(self, flow_id):
        ...
        try:
            self.main.commands.call("replay.client", [self.flow])
...
class Events(RequestHandler):
    def get(self):
        self.write([logentry_to_json(e) for e in self.main.events.data])
...
class Settings(RequestHandler):
    def get(self):
        self.write(dict(
            ...
            mode=str(self.main.options.mode),
            intercept_active=self.main.options.intercept_active,
            intercept=self.main.options.intercept,
            showhost=self.main.options.showhost,
            upstream_cert=self.main.options.upstream_cert,
            rawtcp=self.main.options.rawtcp,
            http2=self.main.options.http2,
            websocket=self.main.options.websocket,
            anticache=self.main.options.anticache,
            anticomp=self.main.options.anticomp,
            stickyauth=self.main.options.stickyauth,
            stickycookie=self.main.options.stickycookie,
            stream=self.main.options.stream_large_bodies,
            contentViews=[v.name.replace(' ', '_') for v in contentviews.views],
            listen_host=self.main.options.listen_host,
            listen_port=self.main.options.listen_port,
            server=self.main.options.server,
        ))
    ...
    def put(self):
        ...
        self.main.options.update(**update)


class Options(RequestHandler):
    def get(self):
        self.write(optmanager.dump_dicts(self.main.options))
...
class SaveOptions(RequestHandler):
    def post(self):
        # try:
        #     optmanager.save(self.main.options, CONFIG_PATH, True)
        # except Exception as err:
        #     raise APIError(400, "{}".format(err))
        pass
