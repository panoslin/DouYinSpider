...
class TestAddons(addonmanager.AddonManager):
    ...
    def trigger(self, event, *args, **kwargs):
        if event == "log":
            self.main.logs.append(args[0])
        super().trigger(event, *args, **kwargs)


class RecordingMain(mitmproxy.main.Main):
...
class context:
    ...
    def __init__(self, *addons, options=None, loadcore=True):
        ...
        self.main = RecordingMain(
            options
        )
        self.options = self.main.options

        if loadcore:
            self.main.addons.add(core.Core())

        for a in addons:
            self.main.addons.add(a)
    ...
    def configure(self, addon, **kwargs):
        ...
        """
        if addon not in self.main.addons:
            self.main.addons.register(addon)
        with self.options.rollback(kwargs.keys(), reraise=True):
            if kwargs:
                self.options.update(**kwargs)
            else:
                self.main.addons.invoke_addon(addon, "configure", {})
    ...
    def invoke(self, addon, event, *args, **kwargs):
        ...
        """
        return self.main.addons.invoke_addon(addon, event, *args, **kwargs)
    ...
    def command(self, func, *args):
        ...
        """
        cmd = command.Command(self.main.commands, "test.command", func)
