...
class Loader:
    ...
    def add_option(
        ...
        """
        if name in self.main.options:
            existing = self.main.options._options[name]
            same_signature = (
    ...
    def add_command(self, path: str, func: typing.Callable) -> None:
        self.main.commands.add(path, func)
...
class AddonManager:
    def __init__(self, main):
        ...
        self.chain = []
        self.main = main
        main.options.changed.connect(self._configure_all)
    ...
    def register(self, addon):
        ...
        for a in traverse([addon]):
            ...
            self.lookup[name] = a
        for a in traverse([addon]):
            self.main.commands.collect_commands(a)
        self.main.options.process_deferred()
        return addon
