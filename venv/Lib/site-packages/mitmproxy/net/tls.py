...
class MainSecretLogger:
    ...
    def __call__(self, connection, where, ret):
        if where == SSL.SSL_CB_HANDSHAKE_DONE and ret == 1:
            with self.lock:
                ...
                mainkey = binascii.hexlify(connection.main_key())
                self.f.write(b"CLIENT_RANDOM %s %s\r\n" % (client_random, mainkey))
                self.f.flush()
    ...
    def create_logfun(filename):
        if filename:
            return MainSecretLogger(filename)
        return None


log_main_secret = MainSecretLogger.create_logfun(
...
def _create_ssl_context(
    ...
    # SSLKEYLOGFILE
    if log_main_secret:
        context.set_info_callback(log_main_secret)
