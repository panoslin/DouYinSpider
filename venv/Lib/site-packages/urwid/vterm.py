...
class Terminal(Widget):
    ...
    def __init__(self, command, env=None, main_loop=None, escape_sequence=None):
        ...
        to act on input from the PTY main device. This object must have
    ...
    def spawn(self):
        ...
        if self.main_loop is None:
            fcntl.fcntl(self.main, fcntl.F_SETFL, os.O_NONBLOCK)
    ...
    def flush_responses(self):
        for string in self.response_buffer:
            os.write(self.main, string.encode('ascii'))
        self.response_buffer = []

    def set_termsize(self, width, height):
        winsize = struct.pack("HHHH", height, width, 0, 0)
        fcntl.ioctl(self.main, termios.TIOCSWINSZ, winsize)
    ...
    def add_watch(self):
        ...
        self.main_loop.watch_file(self.main, self.feed)

    def remove_watch(self):
        if self.main_loop is None:
            return

        self.main_loop.remove_watch_file(self.main)
    ...
    def wait_and_feed(self, timeout=1.0):
        while True:
            try:
                select.select([self.main], [], [], timeout)
                break
